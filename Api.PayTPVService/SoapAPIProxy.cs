...
namespace Api.PayTPVService
{
    internal class SoapAPIProxy
    {
        ...
        /// <returns></returns>
        public static string Execute_purchase_rtoken(string signature ,string merchantCode, string terminal,string amount, 
            string transreference, string rtoken, string currency, string productdescription = "false")
        {
            //execute_purchase_rtoken
            ExampleAPI.PreInvoke();

            ExampleAPI.AddParameter("DS_MERCHANT_MERCHANTCODE", merchantCode);
            ExampleAPI.AddParameter("DS_MERCHANT_TERMINAL", terminal);
            ExampleAPI.AddParameter("DS_MERCHANT_AMOUNT", amount);
            ExampleAPI.AddParameter("DS_MERCHANT_ORDER", transreference);
            ExampleAPI.AddParameter("DS_MERCHANT_IDENTIFIER", rtoken);
            ExampleAPI.AddParameter("DS_MERCHANT_CURRENCY", currency);
            ExampleAPI.AddParameter("DS_MERCHANT_MERCHANTSIGNATURE", signature);
            ExampleAPI.AddParameter("DS_MERCHANT_PRODUCTDESCRIPTION", productdescription);

            try
            {
                ExampleAPI.Invoke("execute_purchase_rtoken");                // name of the WebMethod to call (Case Sentitive again!)
            }
    ...
    public class WebService
    {
        ...
        private void ExtractResult(string methodName)
        {
            try
            {
                ...
                namespMan.AddNamespace("SOAP-ENV", "http://tempuri.org/");

                XElement webMethodResult = ResponseSOAP.Element(ResponseSOAP.FirstNode.Document.FirstNode.ToString()) ;//.XPathSelectElement("//Envelope/Body/", namespMan);
